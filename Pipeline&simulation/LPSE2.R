	...
	sub("seasiarun3t2","seasiarun3t2(4th sample distribution compard)",index.file[1,])
...
######其他方法
	...
	#setwd(index.file[i])
	setwd(sub("seasiarun3t2","seasiarun3t2(4th sample distribution compard)",index.file[i]))
...
######beagle
	...
	root2<-path.expand("~\\imputation\\pipline\\simulation_data\\seasiarun3t2(4th sample distribution compard)\\beagle\\seasiarun3t2(4 sample distribution)")
	setwd(root2)
	...
	setwd(index.file2[i])
	#setwd(sub("seasiarun3t2","seasiarun3t2(4th sample distribution compard)",index.file2[i]))
	p.data<-dir()[grep("GWAS.Results",dir())]
	for(j in 1:length(p.data)){
	setwd(index.file2[i])
	#setwd(sub("seasiarun3t2","seasiarun3t2(4th sample distribution compard)",index.file2[i]))
...
######計算mean和SE
	...
	write.table(yy, file = "statistic_for_LPSE(a.4).txt", sep = ",", row.names = T,quote=F)
