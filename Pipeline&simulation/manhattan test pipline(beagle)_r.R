	...
	root<-path.expand("C:\\Users\\user1\\Documents\\imputation\\pipline\\simulation_data\\seasiarun3t2(4th sample distribution compard)\\beagle\\seasiarun3t2(4 sample distribution)")
	setwd(root)

	########################################################################
	#生產Beagle result index
	#
	dir.result<-matrix(NA,360,1)
	...
	s.file_names<-as.character(s.file.info[6,i])
    	p.s.file_names<-strsplit(s.file_names,"/")[[1]][6:7]
	file_names<-root
	...
	file_name<-paste("Beagle",paste(paste("simulation",rp[rere],sep="_"),"gapit",sep="_"),sep="_")
	...
	index_path<-index.file[(f_rep==repeat_f),1]
	...
	myY<-read.table("C:\\Users\\user1\\Documents\\imputation\\pipline\\simulation_data\\seasiarun3t2\\mydata05.phen", head = TRUE)##### simulation phenotype
