	...
	root<-"C:\\Users\\user1\\Documents\\imputation\\pipline\\simulation_data\\RILsGBSHapmap.f75(4th sample distribution)\\"
	or.root<-"C:\\Users\\user1\\Documents\\imputation\\pipline\\simulation_data\\RIL_answer_for_gabit_05"
	setwd(root)

	index.file<- as.matrix(read.delim("manhattan_file_result.txt",sep=",",head = T))
	sub("RILsGBSHapmap.f75","RILsGBSHapmap.f75(4th sample distribution)",index.file[1,])

	times<-Sys.time()

######其他方法
	
	result<-matrix(NA,350,2)
	count<-1
	for(i in 1:length(index.file)){
	#setwd(index.file[i])
	setwd(sub("RILsGBSHapmap.f75","RILsGBSHapmap.f75(4th sample distribution)",index.file[i]))
	...
	or.data<-as.matrix(read.csv(paste(or.root,p.or.data[p.data[j]==p.or.data],sep="\\")))
	...
	result[count,2]<-sum(((-log10(as.numeric(use.or.data[,4])))-(-log10(as.numeric(use.data[,4]))))^2)/1461
	setwd(root)
...
######beagle
	...
	root2<-path.expand("~\\imputation\\pipline\\simulation_data\\RILsGBSHapmap.f75(4th sample distribution)\\beagle\\RILsGBSHapmap.f75(sample distribution)")
	setwd(root2)
	index.file2<- as.matrix(read.delim("manhattan_file_result.txt",sep=",",head = T))


	#result2<-matrix(NA,250,2)
	result2<-matrix(NA,50,2)
	count<-1
	for(i in 1:length(index.file2)){
	#setwd(index.file2[i])
	setwd(sub("RILsGBSHapmap.f75","RILsGBSHapmap.f75(4th sample distribution)",index.file2[i]))
	p.data<-dir()[grep("GWAS.Results",dir())]
	for(j in 1:length(p.data)){
	#setwd(index.file2[i])
	setwd(sub("RILsGBSHapmap.f75","RILsGBSHapmap.f75(4th sample distribution)",index.file2[i]))
	...
	result2[count,2]<-sum(((-log10(as.numeric(use.or.data[,4])))-(-log10(as.numeric(use.data[,4]))))^2)/1461
	setwd(root)
...
######計算mean和SE
	...
	a<-as.numeric(xx[(met_f==i),2])
	#yy[(2*j-1),i]<-mean(a)
	#yy[(j*2),i]<-(sd(a)/sqrt(length(a)))
	yy[1,i]<-mean(a)
	yy[2,i]<-(sd(a)/sqrt(length(a)))
	#}
	}
	...
	write.table(yy, file = "statistic_for_LPSE(b.4).txt", sep = ",", row.names = T,quote=F)
...
######畫折線圖
	...
	plot(x=c(1,2,3,4,5,6,7,8),y=yy[1,]
	, ylab="LPSE",type="b",lty=1, 
	xlab="",
	pch=1,xaxt='n',lwd=2)
	axis(side=1,at=c(1,2,3,4,5,6,7,8),labels=c("KNN","WKNN","LDKNN","WLDKNN","KNNcat","SVM","PNN","Beagle4"),lwd=1.5)
	abline(v=c(1:8), col='grey', lwd=0.5)
	
	segments(c(1,2,3,4,5,6,7,8),yy[1,]-yy[2,],c(1,2,3,4,5,6,7,8),yy[1,]+yy[2,])
	epsilon <- 0.02
	segments(c(1,2,3,4,5,6,7,8)-epsilon,yy[1,]-yy[2,],c(1,2,3,4,5,6,7,8)+epsilon,yy[1,]-yy[2,])
	segments(c(1,2,3,4,5,6,7,8)-epsilon,yy[1,]+yy[2,],c(1,2,3,4,5,6,7,8)+epsilon,yy[1,]+yy[2,])
	...
	pch=1,ylim=c(0,1500),xlim=c(0.5,5.5),xaxt='n',lwd=2)
	...
	legend(0.5, 1500,c("KNN","WKNN","LDKNN","WLDKNN","KNNcat","SVM","PNN","Beagle4"),pch=1:8,col=c(1,((c(2:8)))),lty=1:8)
	
	#d.data<-data.frame(y=as.numeric(xx[,2]),mis_f=factor(mis_f),met_f=factor(met_f),ph_f=factor(ph_f))
	d.data<-data.frame(y=as.numeric(xx[,2]),met_f=factor(met_f),ph_f=factor(ph_f))
	fit <- aov(y~met_f*ph_f,data=d.data)
