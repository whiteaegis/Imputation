	...
	for(i in 1:50){
		...
		if(missing_flag=="0.01"){match=5;mismatch=-5}
		if(missing_flag=="0.05"){match=4;mismatch=-4}
		if(missing_flag=="0.1"){match=5;mismatch=-5}
		if(missing_flag=="0.5"){match=5;mismatch=-5}
	...
	setwd(root)
	write.table(result, file = "RIL_result_for_score_test_chr01.txt", sep = ",", row.names = FALSE,quote=F)
	}
	Sys.time()-times
